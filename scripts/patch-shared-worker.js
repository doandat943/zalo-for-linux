...
async function main() {
    ...
    if (targetRegex.test(content)) {
        content = content.replace(targetRegex, (match, randomCode) => {
            return 'l.localDttm=Date.now(),l.src=ae.MSG_SRC.SYNC_MOBILE_DB,l.sequenceId=e.sequenseId,l.uidSenderDel=n;const p=e.localPathRaw;"string"==typeof p&&p.length>3&&/^[\x20-\x7E]+$/.test(p)&&(l.localPath=p),l.msgType!==ae.MSG_UNKNOWN?t.push(l):this.logger.zsymb(0,"flow15",(()=>["skip message: unknown type",{';
        });
        fs.writeFileSync(mainstartupFilePath, content, 'utf-8');
        count++;
    ...
    if (content.includes('e=await this._filterDeletedMessages(e);const t=await this._insertMessage(e);')) {
        content = content.replace('e=await this._filterDeletedMessages(e);const t=await this._insertMessage(e);', 
            'e=await this._filterDeletedMessages(e);const t=await this._insertMessage(e);console.error("[SYNC] insertToDb(v2) input=",null==e?void 0:e.length,"ok=",null===(s=t.success)||void 0===s?void 0: s.length, "fail=", null === (r = t.fail) || void 0 === r ? void 0 : r.length);var s,r;')
            fs.writeFileSync(mainstartupFilePath, content, 'utf-8');
        count++;
    ...
    if (content.includes('convertCrossV2ToCrossV1(e){const t=Be[e.MsgType];if(!t)return null;const s=qe.parseBinNet(e.BinNet)||{};if(s.result>0)return this.logger.zsymb(21')) {
        content = content.replace('convertCrossV2ToCrossV1(e){const t=Be[e.MsgType];if(!t)return null;const s=qe.parseBinNet(e.BinNet)||{};if(s.result>0)return this.logger.zsymb(21', 
            'convertCrossV2ToCrossV1(e){const t=Be[e.MsgType]||"webchat",_Buf="undefined"!=typeof Buffer&&Buffer&&Buffer.from?Buffer:("undefined"!=typeof globalThis&&globalThis.Buffer&&globalThis.Buffer.from?globalThis.Buffer:("function"==typeof require?(e=>{try{return require("buffer").Buffer}catch{return null}})():null));let _bin=e.BinNet;try{if(_Buf&&_bin&&!_Buf.isBuffer(_bin)){if("undefined"!=typeof ArrayBuffer&&ArrayBuffer.isView&&ArrayBuffer.isView(_bin)&&_bin.buffer){_bin=_Buf.from(new Uint8Array(_bin.buffer,_bin.byteOffset||0,_bin.byteLength||_bin.length||0))}else if(_bin instanceof Uint8Array)_bin=_Buf.from(_bin);else if(_bin&&_bin.buffer&&"number"==typeof _bin.byteLength)_bin=_Buf.from(new Uint8Array(_bin.buffer,_bin.byteOffset||0,_bin.byteLength));else if(Array.isArray(_bin)&&_bin.length&&"number"==typeof _bin[0])_bin=_Buf.from(_bin);else if(_bin&&"Buffer"===_bin.type&&Array.isArray(_bin.data))_bin=_Buf.from(_bin.data);else if(_bin&&Array.isArray(_bin.data)&&_bin.data.length&&"number"==typeof _bin.data[0])_bin=_Buf.from(_bin.data);else if(_bin&&"number"==typeof _bin.length&&_bin.length>0&&"number"==typeof _bin[0]){const a=new Array(_bin.length);for(let i=0;i<_bin.length;i++)a[i]=_bin[i]&255;_bin=_Buf.from(a)}else if("string"==typeof _bin){const b=_ bin.trim();if(/^[0-9a-fA-F]+$/.test(b)&&b.length%2==0)_ bin=_Buf.from(b,"hex");else if(/^[A-Za-z0-9+/]+={0,2}$/.test(b)&&b.length>=16)_ bin=_Buf.from(b,"base64");else if(b.includes("\\u00")){const unesc=b.replace(/\\u([0-9a-fA-F]{4})/g,((_,h)=>String.fromCharCode(parseInt(h,16))));_ bin=_Buf.from(unesc,"binary")}else _ bin=_Buf.from(b,"binary")}}}catch{}const s=qe.parseBinNet(_ bin)||{};let r=s.data||{};const n=e=>{if(null===e||void 0===e)return"";const t=String(e);return/^g\d+$/.test(t)?t.slice(1):t};const a=[e.SenderId,e.FromId,e.fromId,e.FromUid,e.fromUid,r.fromD,r.uid,r.ownerId].find((e=>null!===e&&void 0!==e&&""!==e)),i=[e.OwnerId,e.ownerId,e.ToId,e.toId,e.ReceiverId,e.ConversationId,this.backupConvId,this.noiseId].find((e=>null!===e&&void 0!==e&&""!==e)),o=null!==a&&void 0!==a?a:this.noiseId;s.result>0&&this.logger.zsymb(21')
            fs.writeFileSync(mainstartupFilePath, content, 'utf-8');
        count++;
